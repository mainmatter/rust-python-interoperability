...
impl ShoppingOrder {
    ...
    fn new(name: String, price: Bound<'_, PyLong>, quantity: Bound<'_, PyLong>) -> PyResult<Self> {
        ...
        let price: u64 = price.extract().map_err(|_| PyValueError::new_err("price must be non-negative"))?;
        if price == 0 {
            return Err(PyValueError::new_err("price cannot be zero"));
        }
        let quantity: u64 = quantity.extract().map_err(|_| PyValueError::new_err("quantity must be non-negative"))?;
        if quantity == 0 {
            return Err(PyValueError::new_err("quantity cannot be zero"));
        }
        Ok(ShoppingOrder { name, price, quantity })
    }
}
